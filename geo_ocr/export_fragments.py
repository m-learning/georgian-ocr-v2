...
def create_char_element(meta_obj, fragment_name):
    ...
    fragment_template = fragment_template.replace('{width}',
                                                  str(meta_obj['w']))
    fragment_template = fragment_template.replace('{height}',
                                                  str(meta_obj['h']))
...
def export_svg(original_image, meta_dir, fragment_dir, output_svg):
    ...
    dir_path = os.path.dirname(os.path.realpath(__file__))
    with open(os.path.join(dir_path,
                           'export_templates/page.svg'), 'r') as content_file:
    ...
    page_template = page_template.replace('{background-image-width}',
                                          str(image_width))
    page_template = page_template.replace('{background-image-height}',
                                          str(image_height))
